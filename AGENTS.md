...
- The host's event history is the durable source of truth for public game state. `persistence.ts` stores host configuration and history together with private digital dealer state (deck and hole cards) in one host-only aggregate so a reload cannot advance one without the other. Clients do not persist game state; after refresh they reconnect and receive a current snapshot.
