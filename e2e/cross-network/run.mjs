...
const appUrl = process.env.CROSS_NETWORK_APP_URL ?? "https://woohp.github.io/poker/";
const settleMs = Number(process.env.CROSS_NETWORK_SETTLE_MS ?? 12_000);
const controlPath = join(tmpdir(), "poker-cross-network-%C");
const remoteDir = "/tmp/poker-cross-network";
...
const tempDir = await mkdtemp(join(tmpdir(), "poker-cross-network-"));
const results = [];
...
try {
    ...
    for (let runNumber = 1; runNumber <= attempts; runNumber++) {
        ...
        try {
            const room = await waitForFile(roomFile, 30_000);
...
function summarizeRun(host, guest) {
    const guestConnects = peerConnectTimes(guest.diagnostics, guest.clickWallTime);
    const hostConnects = peerConnectTimes(host.diagnostics, guest.clickWallTime);
    const providers = ["nostr", "webtorrent"];
    const guestUiMs = guest.playersTwoMonotonicTime - guest.clickMonotonicTime;
    const hostUiMs = host.playersTwoWallTime - guest.clickWallTime;
...
function formatSummary(result) {
    const provider = (name) =>
        Number.isFinite(result.guestConnects[name])
            ...
            : "FAILED";
    return `  UI guest=${Math.round(result.guestUiMs)}ms host=${Math.round(result.hostUiMs)}ms; nostr=${provider("nostr")}; webtorrent=${provider("webtorrent")}; ${result.passed ? "PASS" : "FAIL"}`;
}

function formatAggregate(result) {
    const median = (value) => (value ? `${value.median}ms` : "FAILED");
    return `passed ${result.passed}/${result.attempts}; median guest UI=${median(result.guestUiMs)}, host UI=${median(result.hostUiMs)}, nostr=${median(result.nostrMs)}, webtorrent=${median(result.webtorrentMs)}`;
}
